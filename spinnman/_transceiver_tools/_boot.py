...
from spinnman.scp.scp_message_reset import SCPMessageReset
from spinnman._transceiver_tools import _scamp_constants
from spinnman import exceptions

def _readstruct(section_name, struct_file):
    """ Reads a specific section from a structfile
    ...
    :raise spinnman.exceptions.SpinnmanInvalidParameterException:\ 
                If the struct file is not valid
    """
    sv = dict()
    sv['=size='] = 0
    match = False
                ...
                if equal_match is not None:
                    if equal_match.group(1) == "size":
                        ...
                        raise exceptions.SpinnmanInvalidParameterException(
                                "struct parameter", equal_match.group(1) ,
                                "Not a valid parameter in the structure")
...
def _boot_pkt(connection_socket, host, op, a1, a2, a3, data=None,
    ...
    if data is not None:
        ...
        struct.pack_into(">HLLLL", pkt_data, 0, _scamp_constants.BOOT_PROT_VER,
                         op, a1, a2, a3)
        off = 0
        readsize = datasize
        ...
        while off < readsize:
            ...
            off += 4
        connection_socket.sendto(pkt_data, (host, _scamp_constants.BOOT_PORT))
    else:
        hdr = struct.pack(">HLLLL", _scamp_constants.BOOT_PROT_VER, op, a1,
                          a2, a3)
        connection_socket.sendto(hdr, (host, _scamp_constants.BOOT_PORT))
    time.sleep(_scamp_constants.BOOT_DELAY)
...
def _rom_boot(the_hostname, data):
    ...
    _boot_pkt(sock, host, _scamp_constants.BOOT_CMD_START, 0, 0, block_count - 1)
    offset = 0
    for cur_block in range(0, block_count):
        arg1 = ((_scamp_constants.BOOT_BLOCK_SIZE - 1) << 8) | cur_block
        _boot_pkt(sock, host, _scamp_constants.BOOT_CMD_BLOCK, arg1, 0,  0,
                  data, offset, block_size)
        offset += block_size
    _boot_pkt(sock, host, _scamp_constants.BOOT_CMD_DONE, 1, 0, 0)
...
def boot(the_hostname, the_bootfile, config_file, struct_file):
    ...
    if the_bootfile.endswith(".boot"):
        ...
        raise exceptions.SpinnmanInvalidParameterException(
                "the_bootfile", the_bootfile, "Unknown file extension")
...
def reset(the_hostname):
    ...
    :return: None
    :rtype: None
    :raise spinnman.exceptions.SDPIOException: If there is an error\
                sending the packet
    :raise spinnman.exceptions.SCPIOException: If there is an SCP error\
                sending the packet
    """
    conn = SCPConnection(remote_host=the_hostname)
    msg = SCPMessageReset()
    conn.send(msg)
