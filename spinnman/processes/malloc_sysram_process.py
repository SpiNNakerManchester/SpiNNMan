...
from spinnman.messages.scp.impl.sysram_alloc import SysRAMAlloc, _AllocResponse
from .abstract_multi_connection_process import AbstractMultiConnectionProcess
from .abstract_multi_connection_process_connection_selector import (
    ConnectionSelector)


class MallocSysRAMProcess(AbstractMultiConnectionProcess[_AllocResponse]):
    """
    A process for allocating a block of SystemRAM on a SpiNNaker chip.
    """
    ...
    def __handle_sysram_alloc_response(
    ...
    def malloc_sysram(
        ...
        """
        Allocate space in the SysRAM space.
        ...
        with self._collect_responses():
            self._send_request(
                SysRAMAlloc(x, y, app_id, size, tag),
                functools.partial(self.__handle_sysram_alloc_response, 0))

    def malloc_sysram_multi(
            self, allocations: List[Tuple[int, int, int, int, int]]) -> None:
        """
        Allocate space in the SysRAM space for multiple chips
        ...
        with self._collect_responses():
            for i, (x, y, size, app_id, tag) in enumerate(allocations):
                self._send_request(
                    SysRAMAlloc(x, y, app_id, size, tag),
                    functools.partial(
                        self.__handle_sysram_alloc_response, i))
