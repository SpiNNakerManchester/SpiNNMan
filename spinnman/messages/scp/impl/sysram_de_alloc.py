...
class _SCPSysRAMDeAllocResponse(AbstractSCPResponse):
    """
    An SCP response to a request to deallocate System RAM.
    """
    __slots__ = (
    ...
    def read_data_bytestring(self, data: bytes, offset: int) -> None:
        ...
        if result != SCPResult.RC_OK:
            raise SpinnmanUnexpectedResponseCodeException(
                "SysRAM deallocation", "CMD_DEALLOC", result.name)
        if self._read_n_blocks_freed:
            ...
            if self._number_of_blocks_freed == 0:
                raise SpinnmanUnexpectedResponseCodeException(
                    "SysRAM deallocation response base address", "CMD_DEALLOC",
                    result.name)
...
class SysRAMDeAlloc(AbstractSCPRequest[_SCPSysRAMDeAllocResponse]):
    """
    An SCP Request to free space in the System RAM.
    """
    ...
    def __init__(self, x: int, y: int, *, app_id: Optional[int] = None,
        ...
        :param base_address: The start address in SysRAM to which the block
        ...
        if base_address is not None:
            ...
            super().__init__(
                ...
                argument_1=(
                    AllocFree.FREE_SYSRAM_BY_POINTER.value),
            ...
            super().__init__(
                ...
                argument_1=(
                    app_id << 8 |
                    AllocFree.FREE_SYSRAM_BY_APP_ID.value))
            self._read_n_blocks_freed = True

    @overrides(AbstractSCPRequest.get_scp_response)
    def get_scp_response(self) -> _SCPSysRAMDeAllocResponse:
        return _SCPSysRAMDeAllocResponse(self._read_n_blocks_freed)
