...
class _AllocResponse(AbstractSCPResponse):
    """
    An SCP response to a request to allocate space in System RAM.
    """
    __slots__ = (
        "_base_address",
        "_size")
    ...
    def read_data_bytestring(self, data: bytes, offset: int) -> None:
        ...
        if result != SCPResult.RC_OK:
            raise SpinnmanUnexpectedResponseCodeException(
                "SysRAM Allocation", "CMD_ALLOC", result.name)
        ...
        if self._base_address == 0:
            raise SpinnmanInvalidParameterException(
                "SysRAM Allocation response base address", self._base_address,
                f"Could not allocate {self._size} bytes of System RAM")
...
class SysRAMAlloc(AbstractSCPRequest[_AllocResponse]):
    """
    An SCP Request to allocate space in the SystemRAM space.
    """
    __slots__ = "_size",
    ...
    def __init__(self, x: int, y: int, app_id: int, size: int,
        ...
        :param size: The size in bytes of memory to be allocated
        :param tag:
            The tag for the SysRAM, a 8-bit (chip-wide) tag that can be
