...
class SCPSDRAMDeAllocResponse(AbstractSCPResponse):
    """ An SCP response to a request to de allocate sdram space
    """
    ...
    def read_data_bytestring(self, data, offset):
        ...
        if result != SCPResult.RC_OK:
            raise SpinnmanUnexpectedResponseCodeException(
                "SDRAM de-allocation", "CMD_DEALLOC", result.name)
        self._number_of_blocks_freed = struct.unpack_from("<I", data, offset)[0]
        ...
        if self._number_of_blocks_freed == 0:
            raise SpinnmanUnexpectedResponseCodeException(
                "SDRAM de-allocation response base address", "CMD_DEALLOC",
                result.name)

    @property
    def number_of_blocks_freed(self):
        """ The number of malloced block that have been freed
        from the app_id given
