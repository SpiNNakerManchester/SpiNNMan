...
class EIEIOCommandPortQueuer(AbstractPortQueuer):
    """
    thread that holds a _queue to try to stop the loss of packets from the \
    socket
    """
    ...
    def run(self):
        """
        runs by just putting packets into a non-blocking _queue for the \
        port listener to read from
        """
