...
from spinnman.connections.abstract_classes.udp_receivers.\
    abstract_udp_scp_receiver import AbstractSCPReceiver
...
udp_conn = UDPSpinnakerConnection()
...
class TestSCPListener(unittest.TestCase):
    ...
    def test_new_listener_with_invalid_number_of_error_callback_parameters(
            self):
    ...
    def test_new_listener_with_default_callback_parameters(self):
        SCPListener(udp_conn, MySCPReceiver,
                    print_message_with_default_value,
                    print_errors_with_default_value)
