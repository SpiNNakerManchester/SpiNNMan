...
class TestMessageCallback(unittest.TestCase):
    ...
    def test_wait_for_send_exception(self):
        ...
        with self.assertRaises(EOFError):
            thread.start_new_thread(exception_raised_after_n_seconds,(mc, EOFError,None))
            mc.wait_for_send()

    def test_wait_for_receive(self):
        mc = MC()
        thread.start_new_thread(message_received_after_n_seconds,(mc,"Hello. This is Test speaking."))
        self.assertEqual(mc.wait_for_receive(), "Hello. This is Test speaking.")

    def test_wait_for_receive_exception(self):
        mc = MC()
        with self.assertRaises(MemoryError):
            thread.start_new_thread(exception_received_after_n_seconds,(mc,MemoryError, None))
            mc.wait_for_receive()
